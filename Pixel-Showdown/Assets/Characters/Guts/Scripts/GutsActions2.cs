...
public class GutsActions2 : MonoBehaviour
{
    #region Variables
    Rigidbody2D rb;
    ...
    private IEnumerator Dash()
    {
        ...
        if (Input.GetKeyDown(UserInputs2.currentInputs["Dash"]) && canMove && dashCooldown >= 5f){
            dashCooldown = 0f;
    ...
    private void Attack(){
        float verticalInput = Input.GetKeyDown(UserInputs2.currentInputs["-Y"]) ? -1 : Input.GetKeyDown(UserInputs2.currentInputs["+Y"]) ? 1 : Input.GetAxis("Vertical2");
